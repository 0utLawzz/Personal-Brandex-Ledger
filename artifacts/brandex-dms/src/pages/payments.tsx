      ...
      <ResizablePanelGroup direction="horizontal" className="h-full items-stretch">
              ...
              <table className="w-full text-sm text-left">
                <thead className="text-xs text-slate-500 uppercase bg-slate-50 sticky top-0 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-4 py-2 font-medium text-right">Amount (PKR)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {payments?.map((payment) => (
                    <tr 
                      key={payment.id} 
                      onClick={() => { setSelectedId(payment.id); setIsCreating(false); }}
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5">
                        <div className="font-medium text-slate-900">{payment.clientName}</div>
                      ...
                      <td className={`px-4 py-2.5 text-right font-medium ${payment.type === 'received' ? 'text-green-600' : 'text-slate-900'}`}>
        ...
        <ResizablePanel defaultSize={40}>
          <div className="h-full bg-slate-50 overflow-auto">
            {isCreating ? (
              <PaymentForm mode="create" onCancel={() => setIsCreating(false)} onSuccess={(id) => { setIsCreating(false); setSelectedId(id); }} />
            ) : selectedId ? (
              <PaymentForm id={selectedId} mode="edit" onCancel={() => setSelectedId(null)} onSuccess={() => {}} />
            ) : (
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-lg font-semibold text-slate-900">{mode === 'create' ? 'New Ledger Entry' : 'Entry Details'}</h2>
        {mode === 'edit' && (
              ...
              <FormItem>
                ...
                <Select 
                  onValueChange={(val) => {
                    ...
                    form.setValue("caseId", 0); // reset case
                  }} 
                  value={field.value ? field.value.toString() : undefined}
                  disabled={mode === 'edit'} // Don't allow changing client on edit
                >
                  <FormControl>
                    <SelectTrigger className="bg-white">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} type="number" className="bg-white font-medium" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
